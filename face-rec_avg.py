...
#Several images of the reference face are used for recognition
...
def process_video():
    ...
    """
    # Load images of reference face and get an average embedding
    embeddings = []
    for img in reference_images:
        emb = get_embedding(Image.open(img))
        if isinstance(emb, list):  # If a list is returned, take the first one
            emb = emb[0]
        if emb is not None:  # Add only if embedding is successfully received
            embeddings.append(emb)

    if len(embeddings) == 0:
        raise ValueError("No embedding is obtained for any reference image.")

    face_embedding = torch.stack(embeddings).mean(dim=0)  # average embedding
